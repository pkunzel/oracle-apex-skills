...
Use the VARCHAR2 expression helper for controlled declarative expressions stored in plug-in attributes.

```sql
declare
    l_css_class varchar2(4000);
begin
    l_css_class := apex_plugin_util.get_plsql_expression_result(
        p_plsql_expression => p_region.attributes.get_varchar2('CSS_CLASS_EXPR'),
