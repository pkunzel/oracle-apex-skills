...
Source: [Oracle APEX 26.1 API Reference](https://docs.oracle.com/en/database/oracle/apex/26.1/aeapi/PRINT_ESCAPED_VALUE-Procedure-Signature-2.html)
...
Use the session-state value overload when a plug-in receives an APEX session-state value object.

```sql
begin
    sys.htp.p('<span class="item-value">');
    apex_plugin_util.print_escaped_value(
        p_value => p_value);
    sys.htp.p('</span>');
end;
