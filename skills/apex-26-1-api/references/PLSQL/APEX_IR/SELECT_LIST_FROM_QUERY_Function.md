...
Source: [Oracle APEX 26.1 API Reference](https://docs.oracle.com/en/database/oracle/apex/26.1/aeapi/SELECT_LIST_FROM_QUERY-Function.html)
...
Use this page when code needs the `APEX_IR.SELECT_LIST_FROM_QUERY` function. Confirm security, workspace, and session requirements for your calling context.
...
| `p_query` | SQL query that is expected to select two columns, a display column, and a return column. For example: Note that this is used only by the SELECT_LIST_FROM_QUERY function. Also note, if only one column is specified in the select clause of this query, the value for this column is used for both display and return purposes. |
