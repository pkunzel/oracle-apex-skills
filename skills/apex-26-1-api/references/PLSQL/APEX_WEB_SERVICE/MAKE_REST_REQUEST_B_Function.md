...
Source: [Oracle APEX 26.1 API Reference](https://docs.oracle.com/en/database/oracle/apex/26.1/aeapi/MAKE_REST_REQUEST_B-Function.html)
...
Use this page when code needs the `APEX_WEB_SERVICE.MAKE_REST_REQUEST_B` function. Confirm security, workspace, and session requirements for your calling context.
...
Download a binary response such as a PDF.
...
begin
    l_pdf := apex_web_service.make_rest_request_b(
        p_url                  => 'https://api.example.com/orders/1001/invoice.pdf',
        p_http_method          => 'GET',
        p_credential_static_id => 'ORDERS_API');

    insert into app_files(file_name, mime_type, content_blob)
    values ('invoice-1001.pdf', 'application/pdf', l_pdf);
end;
/
```
