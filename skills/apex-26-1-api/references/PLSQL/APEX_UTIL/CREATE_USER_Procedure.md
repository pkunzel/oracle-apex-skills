...
Create an end-user account for an application that uses APEX Accounts authentication.

```sql
begin
    apex_util.create_user(
        p_user_name                    => upper(:P20_USERNAME),
        p_first_name                   => :P20_FIRST_NAME,
        p_last_name                    => :P20_LAST_NAME,
        p_email_address                => :P20_EMAIL,
        p_web_password                 => :P20_TEMP_PASSWORD,
